...
{
    $totals = [];
    $docFields = getRows($fileName);

    foreach ($docFields as $row) {
        $row = array_map('trim', $row);
        foreach ($row as $code) {
            array_walk($row, function($val, $key) use ($code, &$totals) {
                if ($code != $val) {
                    $divResult = $code / $val;
                    if (is_numeric($divResult)
                        && (is_int($divResult + 0)
                            || (intval($divResult + 0) === intval(ceil($divResult + 0)))))
                    {
                        $totals[] = $divResult;
                    }
                }
...
function cleanSpreadsheetInput(string $input, $fileName)
{
